...
func handleConnection(conn *net.Conn, SolverReportChan *chan *solve.SolverReport) {
    for {
        var one []byte
...
func CollectBest(solverReportChan chan *solve.SolverReport, populationInfluxChan chan []string) {
    for {
...
func wsHandler(w http.ResponseWriter, r *http.Request) {
    ...
    breeder := solve.Breeders(populationInfluxChan, solve.NewCopyBreeder(15), FlappyBreeder{10}, solve.NewRandomBreeder(25, 50, is), solve.NewMutationBreeder(25, 0.1, is), solve.NewCrossoverBreeder(25))
    flappyEval := new(FlappyEvaluator)
    selector := solve.AndSelect(solve.TopX(10), solve.Tournament(10))
    solver := solve.NewSolver(id, &heap, 4, is, terminationCondition, breeder, flappyEval, selector)
    solver.SolverReportChan = SolverReportChan
    id++
    go func() {
        for {
            <-outChan
...
func main() {
    loadProgram("", 0)
    go h.run()
    vis := make(chan *solve.SolverReport)
